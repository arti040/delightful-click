...
;(function ($,window,document) {
  ...
  $.extend(Plugin.prototype, {
    init: function() {
      if(this.settings.destroy == 'self') {
        ...
        if(typeof me.effects[me.settings.effect] === 'function') {
          ...
          console.log("There is not (yet) such effect like ",me.settings.effect);
        }
      }
    },
    ...
    destroy: function(mode) {
      mode == 'all' ? $('.'+this.settings.effectClass).remove() :
      ...
      wave: function(el,opts) {
        ...
        if(opts.run) {
          ...
          $effect[opts.engine]({
            ...
            //Shorthand could be used here but this way is easier to code ;-)
            $effect
            .css('width','1px')
            .css('height','1px')
            .css('margin','auto')
